...
if [ $rc -ne 0 ] || grep -q FATAL $W/log/setup.log || [ ! -x $W/tools/VC3D.AppImage ]; then
  echo "$(date -u +%FT%TZ) setup failed, removing pod" >> $W/h96/progress.log; runpodctl remove pod 2pow0f42k6bzxs || runpodctl stop pod 2pow0f42k6bzxs; exit 1
fi
POD=2pow0f42k6bzxs bash $W/hecate_all.sh
