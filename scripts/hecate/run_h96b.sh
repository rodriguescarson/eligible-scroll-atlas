...
if [ $rc -ne 0 ] || grep -q FATAL $W/log/setup.log || [ ! -x $W/tools/VC3D.AppImage ]; then
  echo "$(date -u +%FT%TZ) setup failed, removing pod" >> $W/h96/progress.log; runpodctl remove pod o72grzisiwnxic || runpodctl stop pod o72grzisiwnxic; exit 1
fi
POD=o72grzisiwnxic bash $W/hecate_all.sh
